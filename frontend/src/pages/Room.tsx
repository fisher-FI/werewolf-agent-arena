...
export default function Room() {
    ...
    const ws = connectWS(roomId, (msg) => {
      switch (msg.type) {
        ...
        case 'phase_change':
          ...
          break;
        case 'game_start':
          setRoom((prev: any) => prev ? { ...prev, status: 'playing', players: msg.data.players } : prev);
          setEvents([]);
          setReasonings([]);
          break;
        case 'game_end':
  ...
  const getTeam = (role: string | null | undefined) => role === 'werewolf' ? 'werewolf' : 'villager';
          ...
          <>
            ...
            {hasRole && (
              <div className="seat-role">
                ...
                <span className="role-name">{ROLE_LABEL[player.role]}</span>
                <span className={`role-team team-${getTeam(player.role)}`}>
                  {getTeam(player.role) === 'werewolf' ? '狼人阵营' : '好人阵营'}
                </span>
              </div>
            )}
            {isWaiting && (
  ...
  const renderEvent = (evt: any, idx: number) => {
    ...
    return (
      <div key={idx} className={`timeline-event event-${type}`} onClick={() => p && setSelectedPlayer(evt.player_id)}>
        ...
        {type === 'vote_result' && (
          <div className="evt-vote-result">📊 {evt.content}</div>
        )}
        {type === 'player_death' && (
          <div className="evt-death">💀 {evt.content}</div>
        )}
        {type === 'game_end' && (
          <div className="evt-end">🏆 {evt.content}</div>
        )}
        {type === 'werewolf_kill' && (
          <div className="evt-wolf-kill">🐺 {evt.content}</div>
        )}
        {type === 'seer_check' && (
          <div className="evt-seer">🔮 {evt.content}</div>
        )}
        {type === 'witch_save' && (
          <div className="evt-witch">🧪 {evt.content}</div>
        )}
        {type === 'witch_poison' && (
          <div className="evt-witch">☠️ {evt.content}</div>
        )}
        {type === 'hunter_shoot' && (
          <div className="evt-hunter">🔫 {evt.content}</div>
        )}
        {type === 'system' && (
          <div className="evt-system">{evt.content}</div>
        )}
        {!['player_speech','vote_cast','vote_result','player_death','game_end','werewolf_kill','seer_check','witch_save','witch_poison','hunter_shoot','system','phase_change'].includes(type) && (
          <div className="evt-system">{evt.content}</div>
        )}
      </div>
    );
    ...
    <div className="room-page">
        ...
        <div className="room-actions">
          {isWaiting && (
            <>
              ...
              <button className="btn-primary" onClick={startGame} disabled={playerCount < 6}>
                开始游戏 ({playerCount}/9)
              </button>
            </>
          )}
      ...
      {(selectedPlayer && playerMap[selectedPlayer]) ? (
            ...
            {playerMap[selectedPlayer]?.role && (
              <span className="pdb-role">
                ...
                <span className={`role-team team-${getTeam(playerMap[selectedPlayer].role)}`}>
                  {getTeam(playerMap[selectedPlayer].role) === 'werewolf' ? '狼人' : '好人'}
                </span>
              </span>
            )}
        ...
        <div className="gameover-bar">
          <span className="gameover-title">🏆 {gameOver.winner_label} 获胜！</span>
